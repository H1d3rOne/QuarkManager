...
class ShareService:
    ...
    def save_shared_files(
        ...
        token: str,
        file_ids: List[str],
        share_fid_tokens: List[str] = None,
        ...
        Args:
            ...
            token: 访问令牌
            file_ids: 要转存的文件ID列表
            share_fid_tokens: 文件对应的share_fid_token列表（与file_ids一一对应）
        ...
        Returns:
            转存结果
        """
        # 如果没有提供 share_fid_tokens，使用空列表
        if share_fid_tokens is None:
            share_fid_tokens = []
        ...
        # 调试：打印请求数据
        import json
        print(f"[DEBUG] save_shared_files 请求数据: {json.dumps(data, ensure_ascii=False, indent=2)}")
    ...
    def parse_and_save(
        ...
        self.logger.info(f"[parse_and_save] share_id: {share_id}, password: {password}")
        
        # 2. 获取访问令牌
        token = self.get_share_token(share_id, password)
        self.logger.info(f"[parse_and_save] token: {token}")
        
        # 3. 获取分享信息
        share_info = self.get_share_info(share_id, token)
        self.logger.info(f"[parse_and_save] share_info: {share_info}")
        ...
        self.logger.info(f"📦 要转存的文件ID: {file_ids}")
