...
class FileUploadService:
    ...
    def upload_folder(
        ...
        Args:
            folder_path: 本地文件夹路径
            parent_folder_id: 远程父文件夹ID
            create_parent_folder: 是否先在目标目录创建同名父文件夹
            progress_callback: 进度回调，签名为 callback(event_type, data)
        ...
        if create_parent_folder:
            target_folder_id, is_created = self._get_or_create_folder(
            ...
            if progress_callback:
                progress_callback('folder_created', {
                    'local_path': str(folder_path_obj),
                    'remote_parent_id': parent_folder_id,
        ...
        stats = {
            ...
            'uploaded_files': 0,
            'failed_files': []  # type: List[Dict[str, str]]
        }

        self._upload_folder_recursive(
            local_folder=folder_path_obj,
            remote_folder_id=target_folder_id,
            stats=stats,
            progress_callback=progress_callback
        )

        status = 'success' if not stats['failed_files'] else 'partial_success'
        return {
            'status': status,
            'folder_name': folder_path_obj.name,
    ...
    def _upload_folder_recursive(
        ...
        """递归上传本地文件夹内容到远程文件夹"""
        entries = sorted(local_folder.iterdir(), key=lambda entry: (entry.is_file(), entry.name.lower()))

        for entry in entries:
            if entry.is_dir():
                sub_folder_id, is_created = self._get_or_create_folder(entry.name, remote_folder_id)
                if is_created:
                ...
                if progress_callback:
                    progress_callback('folder_created', {
                        ...
                        'remote_parent_id': remote_folder_id,
                        'remote_folder_id': sub_folder_id,
                        'name': entry.name,
                    })
            ...
            try:
                file_result = self.upload_file(
                    file_path=str(entry),
                    parent_folder_id=remote_folder_id
                )
                ...
                if progress_callback:
                    progress_callback('file_error', error_info)

    def _get_or_create_folder(self, folder_name: str, parent_folder_id: str) -> Tuple[str, bool]:
        """在指定父目录下获取或创建文件夹，返回 (folder_id, is_created)"""
        params = {
        ...
        response = self.api_client.get('file/sort', params=params)
        files_list: List[Dict[str, Any]] = []

        files_data = response.get('data', {})
        if isinstance(files_data, dict):
            if isinstance(files_data.get('list'), list):
                files_list = files_data.get('list', [])
            elif isinstance(files_data.get('data'), dict):
                files_list = files_data.get('data', {}).get('list', [])

        for item in files_list:
            is_folder = item.get('dir', False) or item.get('file_type') == 0
            if is_folder and item.get('file_name') == folder_name and item.get('fid'):
                return item['fid'], False
        ...
        create_response = self.api_client.post('file', json_data=create_data, params=create_params)

        folder_id = create_response.get('data', {}).get('fid')
        if not folder_id:
            raise APIError(f"创建文件夹失败: {folder_name}")
