...
def upload_file(file_path: str, parent_folder_id: str = "0", folder_path: Optional[str] = None, create_dirs: bool = False):
    ...
    try:
        ...
        if not file_path_obj.exists():
            print_error(f"路径不存在: {file_path}")
            raise typer.Exit(1)

        if file_path_obj.is_dir():
            upload_folder(
                folder_path=str(file_path_obj),
    ...
    try:
        ...
        with get_client() as client:
            ...
            if target_folder_path:
                try:
                    parent_folder_id = _resolve_folder_path(client, target_folder_path, create_dirs)
                    print_info(f"目标文件夹: {target_folder_path}")
                except Exception as e:
                    print_error(f"解析文件夹路径失败: {e}")
                    raise typer.Exit(1)

            print_info(f"开始递归上传文件夹: {folder_path_obj.name}")

            def progress_callback(event_type: str, data: dict):
                if event_type == 'folder_created':
                    print_info(f"📁 创建文件夹: {data.get('name', '')}")
                elif event_type == 'file_uploaded':
                    print_info(f"✅ 上传文件: {Path(data.get('local_path', '')).name}")
                elif event_type == 'file_error':
                    print_warning(f"❌ 上传失败: {data.get('local_path', '')} - {data.get('error', '')}")
            ...
            if result.get('status') in ('success', 'partial_success'):
                print_success(f"文件夹上传完成: {folder_path_obj.name}")
                print_info(f"统计: 文件 {uploaded_files} 个，文件夹 {created_folders} 个")
                if failed_files:
                    print_warning(f"失败文件数: {len(failed_files)}")
            else:
                print_error("文件夹上传失败")
                raise typer.Exit(1)
